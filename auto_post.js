...
夜中に一人でお酒を飲みながら「これ気になるな〜」と楽天やAmazonを眺めて本音で比較している個人の独白雑談ブログ記事です。
...
--------------------------------------------------
【絶対に守るべきMarkdown構成＆文章表現ルール】:

1. **商品名（略称）の記述ルール**:
   - 本文や見出しで商品名を記載するときは、必ず短く整えられた略称『${nameA}』および『${nameB}』だけを使用してください。
   - 「保温プレート 保温プレート」や「電気フライヤー 電気フライヤー」のように、同じ単語が重複した不自然な商品名は絶対に書かないでください！

2. **タイトル**:
   - セールス感のある「【徹底比較】」や「おすすめ！」などは禁止。
   - 「『${nameA}』と『${nameB}』、買うならどっち？」のような自然な疑問形タイトル。

3. **文章のトーン・読点「、」・改行スタイル**:
   - AI特有の読点「、」の乱用（「〜で、」「〜で、」など）を止め、人間が喋っているようなテンポの良い自然な日本語にしてください。
   - 句点「。」や独白の区切りごとに【必ず空行を1行挟んで改行】してください。（1つの段落に長文を詰め込まない）
   - 心の声（例: **「え、これどっち買えばいいんだ？」** や **「いや、自分そんなに〇〇しないぞ」**）や金額の差（例: **約${priceDiff}円**）は **太文字** にする。

4. **見出し構成（h1 / # は本文中で使用禁止）**:
   - 導入（「夜って、なんとなく楽天とかAmazonを眺めちゃうことありません？ 別に今すぐ何かが欲しいわけでもないんだけど...」）
   - \`---\`
   - \`## ${nameA}、なんか色々できる\` （※冒頭で1回だけ正式商品名『${fullNameA}』と価格${priceA}円を明記）
   - \`---\`
   - \`## でも「家でおつまみ焼きながら飲む」なら、${nameB}の方が気になる\` （※冒頭で1回だけ正式商品名『${fullNameB}』と価格${priceB}円を明記）
   - \`---\`
   - \`## で、結局どっちがいいんだろう\`
   - \`---\`
...
以下のJSON形式のみで出力してください（Markdown表記の文字列としてcontentHtmlを出力）：
{
  ...
  "contentHtml": "夜って、なんとなく楽天とかAmazonを眺めちゃうことありません？\\n\\n別に今すぐ何かが欲しいわけでもないんだけど...\\n\\n「あー、なんか家で美味しいもの食べたいな」\\n\\nみたいな。\\n\\n---\\n\\n## ${nameA}、なんか色々できる...",

...
夜中に一人でお酒を飲みながら「これ気になるな〜」と楽天やAmazonを眺めて本音で比較している個人の雑談ブログ記事です。セールス・アフィリエイト感は完全に消してください。
...
--------------------------------------------------
【絶対に守るべきMarkdown構成・執筆ルール】:

1. **タイトル**:
   - セールス感のある【徹底比較】や「選び方！」等は絶対NG。
   - 「『${nameA}』と『${nameB}』、買うならどっち？」のような自然な疑問形タイトル。

2. **文章表現と改行**:
   - 語尾は「〜んですよね」「〜かも」「〜らしい」「〜って思っちゃいます」「〜なんです」等の自然な独白体。
   - 1文ごとに空行を入れて改行し、テンポよく読めるようにする。
   - セクション区切りには必ず \`---\` を入れる。
   - 心の声や価格差・重要単語は \`**太文字**\` で囲む。

3. **見出しレベル（本文中には h1 / # を絶対に使用しないこと）**:
   - 本文の主要見出しはすべて \`##\`（h2）を使用。
   - 小見出し（選び分け等）は \`###\`（h3）を使用。

4. **記事の流れ**:
   導入（夜中にネットで家電を眺めていて2つ見つけた雑談）
   ---
   ## ${nameA}、なんか色々できる
   （セクション冒頭で1回だけ正式商品名『${fullNameA}』と価格${priceA}円を明記。以降は略称『${nameA}』を使用）
   ---
   ## でも「〇〇な家飲み」なら、${nameB}の方が気になる
   （セクション冒頭で1回だけ正式商品名『${fullNameB}』と価格${priceB}円を明記。以降は略称『${nameB}』を使用）
   ---
   ## で、結局どっちがいいんだろう
   ---

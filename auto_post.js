...
function convertToCleanHtml(rawContent) {
  ...
  // Amebaブログの仕様上、本文内の <h1> はすべて <h2> に落とす（<h1>はブログタイトル専用のため）
  html = html
    .replace(/<h1[^>]*>/gi, '<h2 style="font-size: 20px; font-weight: bold; margin: 25px 0 15px 0; border-bottom: 2px solid #333; padding-bottom: 8px;">')
    .replace(/<\/h1>/gi, '</h2>')
    .replace(/<hr\s*\/?>/gi, '<hr style="border: none; border-top: 1px solid #ccc; margin: 30px 0;">')
    .replace(/<h2[^>]*>/gi, '<h2 style="font-size: 19px; font-weight: bold; margin: 20px 0 10px 0; background: #f7f7f7; padding: 10px; border-left: 5px solid #ff6600;">')
    .replace(/<h3[^>]*>/gi, '<h3 style="font-size: 17px; font-weight: bold; margin: 15px 0 8px 0; border-left: 3px solid #ff6600; padding-left: 8px;">');
...
深夜に一人でお酒を飲みながら「これ気になるな〜」と楽天やAmazonを眺めて本音で比較している個人の雑談ブログ記事です。アフィリエイト感を出さないでください。
...
--------------------------------------------------
【絶対に守るべきタイトル＆構成・見出しルール】:

1. **タイトル（アフィアフィした【徹底比較】や「選び方！」等は絶対NG）**:
   - exampleのような自然な疑問形タイトルにしてください。
   - タイトル例: 「『${nameA}』と『${nameB}』、買うならどっち？」

2. **見出し・構文（h1は絶対に使用しないこと！タイトル欄と重複するため）**:
   - **本文中で <h1> や \`#\` は絶対に使わないでください（すべて <h2> 以下の見出しにすること）。**
   - セクションの大きな切り替わりには <h2>（例: \`## で、結局どっちがいいんだろう\`、\`## 約${priceDiff}円の差、どう考える？\`、\`## まとめ\`）を使用する。
   - サブセクションには <h3>（例: \`### ${nameA}を選ぶなら\`、\`### ${nameB}を選ぶなら\`）を使用する。
   - 段落の区切りには必ず \`---\`（水平線）を入れる。
   - 太文字 \`**text**\` は、商品名、心の声（例: **「え、これどっち買えばいいんだ？」**）、重要なキーワードや価格差（例: **約${priceDiff}円**）に使う。

3. **文章のトーン・雰囲気（深夜の雑談・独白体）**:
   - 「夜って、なんとなく楽天とかAmazonを眺めちゃうことありません？」というような、深夜のネット巡回・おうち飲み目線の導入から入る。
   - 語尾は「〜んですよね」「〜かも」「〜らしい」「〜って思っちゃいます」「〜なんです」といった自然な独白・雑談体（AI臭さ・セールス臭さを完全に消す）。
   - 句点の後に改行（<br><br>）を多用し、テンポよく読めるようにする。

4. **構成の流れ**:
   - 導入（夜中にネットを見ていて2つの家電が気になった雑談）
   - \`---\`
...
5. **アフィリエイトリンク・プレースホルダーの禁止**:
   - アフィリエイトURLや「ここにリンク」などの文字は絶対に含めない。

--------------------------------------------------
以下のJSON形式のみで出力してください（Markdownコードブロック表記不可）：
{
  "title": "『${nameA}』と『${nameB}』、買うならどっち？",
  "contentHtml": "夜って、なんとなく楽天とかAmazonを眺めちゃうことありません？<br><br>別に今すぐ何かが欲しいわけでもないんだけど...",

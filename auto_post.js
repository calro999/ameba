...
async function fetchRakutenItemPair(primaryObj) {
  ...
  if (!itemA || !itemB) {
    ...
    const altPool = sameSubSubPool = sameSubPool.filter(k => k !== primaryKeyword);
...
5. **内容量（容量・サイズ・個数など）への自然な言及**:
   - 正式商品名やスペックに記載されている内容量やタイプ（例：「Aは〇〇g、Bは〇〇g」「Aは4種類アソート、Bは大粒タイプ」「Aは四合瓶720ml、Bも720ml」など）に軽く触れてください。
...
6. **価格・価格差の扱い（※価格差0円の場合は特に注意！）**:
   ...
   - 「どちらも同じ${priceA}円だからこそ、値段の損得ではなく純粋に『内容量のボリューム』『味の系統・素材へのこだわり』『ペアリング』で真剣に悩む」というスタンスで語ってください。
   ` : `
   - 価格差はわずか約${priceDiff}円とほぼ同価格帯です。
   - 「高いから悩む」「安いからこっち」という損得勘定ではなく、「価格差は約${priceDiff}円とほぼ同じだからこそ、純粋に味の好みや内容量、ペアリングで選べるのが逆に悩ましい」というスタンスで語ってください。
   `}
...
--------------------------------------------------
【Markdown見出し構成ルール】
- 記事タイトルは魅力的でブログらしいものにすること（例: 『${nameA}』と『${nameB}』、次に晩酌で選ぶならどっち？ / 【本気で迷い中】『${nameA}』VS『${nameB}』... 等）
...
- 構成案（各セクションの間は \`---\` で区切る）：
  ...
  - \`---\`
  ${isSamePrice 
    ? `  - \`## どちらも同じ${priceA}円。値段が全く同じだからこそ内容量やこだわりで迷う\`（同額だからこそ内容量・仕様・素材へのこだわりで比較）`
    : `  - \`## 価格差は約${priceDiff}円。値段がほぼ同じだからこそ純粋に悩む\`（同価格帯だからこそ味や内容量、特徴で迷うスタンス）`}
  - \`---\`
...
レビューを見ていると評判が良くて、内容量のボリューム感や味わいも自分の好みに合いそうなんですよね。

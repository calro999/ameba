...
提供された見出し構成（h2, h3）および商品名の記述ルールを100%厳格に守って執筆してください。
...
--------------------------------------------------
【絶対に守るべき見出し構成・商品名記述ルール】:

1. **タイトル（スーパーフックタイトル）**:
   - 読者の興味を強く惹きつける28〜35文字のタイトル（例: 「【徹底比較】${nameA} vs ${nameB}！約${priceDiff}円差で後悔しない選び方」）

2. **構成とHTML見出しタグ（h2, h3を正確に使用）**:

   - **導入**: 自宅での悩み・きっかけ（スマホで読みやすいよう句点の後に <br><br> を多用）

   - **<h2> 候補に挙がったのは、「${nameA}」と、「${nameB}」の2つ。</h2>**
     - 「価格や使い勝手、実際の生活シーンでどっちが本当に活躍するのか、ちょっと本音で比べてみようかなと思います。」等のつなぎ文章。

   - **<h3> 💡 ${nameA} の特徴と魅力</h3>**
     - **【超重要ルールの適用】**: セクション冒頭で「まず気になったのが、こちらの『${fullNameA}』です。価格は${priceA}円。」と、**一度だけ正式商品名＋価格を全文で正確に記述してください**。
     - **以降の本文中では『${nameA}』という略称のみを使用し、価格は再記述しないでください**。
     - 特徴、メリット、向いている人を自然な会話調で解説。

   - **<h3> 💡 ${nameB} の特徴と魅力</h3>**
     - **【超重要ルールの適用】**: セクション冒頭で「一方で、気になったのが『${fullNameB}』です。価格は${priceB}円。」と、**一度だけ正式商品名＋価格を全文で正確に記述してください**。
     - **以降の本文中では『${nameB}』という略称のみを使用し、価格は再記述しないでください**。
     - 特徴、メリット、向いている人を自然な会話調で解説。

   - **<h2> ⚖️ 徹底比較！洗やすさ・機能・後片付けのリアルな差</h2>**
     - 「さて、この2つのアイテム、価格差は約${priceDiff}円。」から開始。
     - ① **お手入れ・洗やすさの差**（パーツ丸洗い、刃の洗いやすさなど）
     - ② **機能・作れるメニューの差**
     - ③ **約${priceDiff}円の価格差を出す価値があるかの本音判定**
     - 本文中はすべて略称『${nameA}』『${nameB}』を使用。

   - **<h2> 🍶 結論：あなたにピッタリなのはどっち？</h2>**
     - 「さて、さんざん比較してきましたが、私ならどっちを選ぶか……。」
     - 商品Aが向いている人への提案: 「〜という方なら、『${fullNameA}』（${priceA}円）が間違いなくおすすめです。」（※ここで正式商品名＋価格を記載）
     - 商品Bが向いている人への提案: 「〜という方なら、『${fullNameB}』（${priceB}円）を選びますね。」（※ここで正式商品名＋価格を記載）
     - 締めの言葉。

3. **アフィリエイトリンク・プレースホルダーの禁止**:
   - アフィリエイトURLや「ここにリンク」などの文字は絶対に書かないでください。

--------------------------------------------------
以下のJSON形式のみで出力してください（Markdownコードブロック表記不可）：
{
  "title": "強烈なスーパーフックタイトル",
  "contentHtml": "<p>導入文...</p><br><br><h2>候補に挙がったのは...</h2>...",
